...
const SignUpOng = () => {
  ...
  const handle = (e) => {
    ...
    if (name === "categorias") {
      setForm((f) => {
        const arr = f.categorias.includes(value)
          ? f.categorias.filter((c) => c !== value)
          : [...f.categorias, value];
        return { ...f, categorias: arr };
      });
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${base}/auth/register/ong`, {
        method: "POST",
    ...
    <div className="signup-wrapper">
      ...
      <div className="card">
        ...
        <div className="toggle">
          <Link to="/signup/donor">Sou Doador</Link>
          <Link to="/signup/ong" className="active">Sou ONG</Link>
        </div>
        <form onSubmit={handleSubmit}>
          <div className="grid">
            ...
            <div className="field">
              <label>CNPJ *</label>
              <input name="cnpj" required onChange={handle} />
            </div>
            <div className="field">
              <label>Nome do Responsável *</label>
              <input name="responsavel" required onChange={handle} />
            </div>
            <div className="field">
              <label>E-mail *</label>
            ...
            <div className="field full">
              ...
              <textarea
                name="descricao"
                rows={3}
                placeholder="Conte sobre a missão e atividades da ONG"
                onChange={handle}
              />
            </div>
            <div className="field full">
              <label>Categorias de Doação Aceitas *</label>
          ...
          <div className="actions">
            <button type="submit" disabled={loading}>
              {loading ? "Criando..." : "Criar Conta como ONG"}
            </button>
          </div>
          <div className="foot">
